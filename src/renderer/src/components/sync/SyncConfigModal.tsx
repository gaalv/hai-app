...
export function SyncConfigModal({ onClose }: Props): JSX.Element {
  ...
  async function handleConnect(): Promise<void> {
    ...
    setError(null)
    try {
      await syncService.configure(repoUrl.trim())
      onClose()
    } catch (err) {
  ...
  const inputCls = "w-full px-3 py-2.5 bg-[var(--surface-3)] text-[var(--text)] border border-[var(--border-2)] focus:border-[var(--accent)] rounded-md text-sm outline-none transition-colors"

  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-[100]">
      <div className="bg-[var(--surface)] border border-[var(--border-2)] rounded-xl p-8 w-[440px]">
        <h2 className="text-base font-semibold text-[var(--text)] mb-2">Configurar GitHub Sync</h2>
        <p className="text-xs text-[var(--text-3)] mb-5 leading-relaxed">
          O sync usa o token do GitHub configurado na autenticação.
          Certifique-se de estar logado com GitHub antes de configurar.
        </p>

        <label className="block text-xs text-[var(--text-3)] mb-1.5">URL do repositório GitHub</label>
        <input
          className={inputCls}
          ...
          autoFocus
        />
        <p className="text-[11px] text-[var(--text-3)] mt-1.5 mb-4">
          O repositório deve existir e ser acessível pela sua conta GitHub.
        </p>
