...
export function QuickCapture(): JSX.Element {
  ...
  async function handleSave(): Promise<void> {
    ...
    try {
      ...
      const title = firstLine || `Capture ${new Date().toLocaleTimeString('pt-BR')}`
      const content = `# ${title}\n\n${text}`

      const filePath = await window.electronAPI.notes.create(inboxAbsPath, title)
  ...
  return (
    <div className="flex flex-col h-screen bg-[#111] text-neutral-200 overflow-hidden rounded-xl">
      ...
      <div className="flex items-center gap-2 px-4 py-2.5 border-b border-[#222] shrink-0">
        <span className="text-xs text-purple-400 font-semibold tracking-widest">hai</span>
        <span className="text-xs text-neutral-500 ml-1">→ Inbox</span>
        <div className="flex-1" />
        <span className="text-[10px] text-neutral-600">⌘↩ salvar · Esc cancelar</span>
      </div>
